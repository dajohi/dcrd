...
func (netAddr *NetAddress) ipString() string {
	var netIP [32]byte
	copy(netIP[:], netAddr.IP)

	switch netAddr.Type {
	case TORv3Address:
		addrBytes := netIP[:]
		checksum := calcTORv3Checksum(netIP)
	...
	default:
		return net.IP(netAddr.IP).String()
	}
}
...
func checkNetAddressType(netAddressType NetAddressType, addrBytes []byte) error {
...
func (a *AddrManager) newAddressFromString(addr string) (*NetAddress, error) {
	...
	return NewNetAddressByType(addrType, addrBytes, uint16(port), timestamp,
		wire.SFNodeNetwork)
}
